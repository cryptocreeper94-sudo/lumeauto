...
const SUBSYSTEMS = [
  ...
  { icon: <Radio size={18}/>, title: 'Voice Intelligence (Optional)', items: ['LLM-agnostic bidirectional voice engine', '15+ hands-free commands', 'All output governed by LUME-V wrapper', 'Speech pattern learning per driver', 'Noise cancellation for lot environments', 'Can be disabled per-facility'], color: '#38bdf8' },
  ...
  { icon: <FileCode size={18}/>, title: 'TrustShield Certification', items: ['AI agent output attestation', 'On-chain verification anchoring', 'Cross-system integrity validation', 'Zero-trust architecture enforcement', 'Deterministic audit trail for every subsystem', 'Enterprise-grade security certification layer'], color: '#38bdf8' },
              ...
              <div key={i} style={{ padding: '0.65rem', background: 'rgba(0,0,0,0.3)', borderRadius: '8px', fontFamily: 'var(--font-mono)' }}>
                <div style={{ display: 'flex', gap: '6px', alignItems: 'center', marginBottom: '0.25rem' }}>
                  <span style={{ fontSize: '0.55rem', padding: '1px 5px', borderRadius: '3px', fontWeight: 700,
                    background: api.method === 'POST' ? 'rgba(16,185,129,0.15)' : api.method === 'WS' ? 'rgba(56,189,248,0.15)' : 'rgba(56,189,248,0.15)',
                    color: api.method === 'POST' ? 'var(--accent-emerald)' : api.method === 'WS' ? '#38bdf8' : 'var(--accent-cyan)',
                  }}>{api.method}</span>
