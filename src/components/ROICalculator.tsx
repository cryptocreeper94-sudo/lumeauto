...
export default function ROICalculator() {
  ...
  return (
    <div className="panel" style={{ padding: '2.5rem', background: 'rgba(16, 185, 129, 0.03)', borderColor: 'rgba(16, 185, 129, 0.2)' }}>
      <h3 style={{ fontSize: '1.5rem', marginBottom: '1.5rem' }}>Interactive Financial Impact</h3>
      
      <div style={{ marginBottom: '2rem' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '1rem' }}>
          <label style={{ fontSize: '0.9rem', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Daily Intake Volume (per location)</label>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2rem' }}>
        <motion.div key={volume} initial={{ scale: 0.95, opacity: 0.5 }} animate={{ scale: 1, opacity: 1 }}>
          <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)', marginBottom: '0.5rem', textTransform: 'uppercase' }}>Inspector Hours Saved / Mo</div>
          <div style={{ fontSize: '2.5rem', fontWeight: 700, color: 'var(--text-main)' }}>{hoursSaved.toLocaleString()}</div>
          <div style={{ fontSize: '0.85rem', color: 'var(--accent-cyan)' }}>Reallocated to reconditioning</div>
        </motion.div>

        <motion.div key={`arb-${volume}`} initial={{ scale: 0.95, opacity: 0.5 }} animate={{ scale: 1, opacity: 1 }}>
          <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)', marginBottom: '0.5rem', textTransform: 'uppercase' }}>Arbitration Savings / Mo</div>
          <div style={{ fontSize: '2.5rem', fontWeight: 700, color: 'var(--accent-emerald)' }}>${arbitrationSavingsMonth.toLocaleString()}</div>
          <div style={{ fontSize: '0.85rem', color: 'var(--accent-cyan)' }}>Direct bottom-line recovery</div>
        </motion.div>
      </div>
    </div>
  );
