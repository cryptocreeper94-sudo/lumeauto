...
const components = [
  {
    ...
    desc: 'Overhead energy transmitter mounted in existing canopy infrastructure. Broadcasts deterministic power packets.',
    detail: 'Each anchor node operates as an autonomous energy transmitter embedded in the canopy superstructure. The node does not negotiate — it broadcasts at a fixed frequency governed by the Meridian organism. Power delivery is deterministic: same input conditions produce identical output every time. Nodes require no active cooling and operate maintenance-free for 15+ years. A single canopy can support 12–24 anchor nodes.',
    color: '#38bdf8',
  },
  {
    icon: <Zap size={24}/>,
    title: 'Wireless Energy Router',
    desc: 'Deterministic protocol engine that routes kWh packets from anchor nodes to vehicle receivers.',
    detail: 'The router does not use WiFi, Bluetooth, or any probabilistic wireless protocol. Energy is routed through resonant inductive coupling at frequencies governed by the organism. Each packet of energy is metered, timestamped, and sealed before delivery. The router handles multi-vehicle simultaneous charging without contention — each vehicle receiver operates on an isolated frequency slot assigned by the organism.',
    color: '#dc2626',
  },
  {
    icon: <Cpu size={24}/>,
    title: 'Vehicle Receiver Plate',
    desc: 'Resonant receiver mounted on vehicle roof or undercarriage. Converts wireless energy to DC charging current.',
    detail: 'The receiver plate is a passive resonant coil tuned to the anchor node frequency. No active electronics on the vehicle side — the plate converts electromagnetic energy to DC current through rectification. Efficiency: 92–96% at optimal alignment. The plate tolerates ±30cm misalignment without efficiency degradation. Installation is a bolt-on retrofit requiring no vehicle modification.',
    color: '#38bdf8',
  },
  {
    icon: <Shield size={24}/>,
    title: 'Cryptographic Billing Engine',
    desc: 'On-chain metering and receipt generation. Every kWh is sealed, timestamped, and publicly verifiable.',
    detail: 'Every energy transfer event generates a cryptographic receipt anchored to the Cox Automotive Ledger. The receipt contains: kWh delivered, duration, vehicle VIN, anchor node ID, and a SHA-256 hash of the metering data. Receipts are publicly verifiable without login or API key. Billing disputes are resolved by mathematical replay — feed the same inputs, get the same receipt. No human judgment required.',
