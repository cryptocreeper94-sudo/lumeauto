...
export default function CoxLedgerSection() {
  return (
    <section style={{ padding: '6rem 0', position: 'relative', overflow: 'hidden', background: 'var(--bg-dark)', borderBottom: '1px solid var(--border-light)' }}>
      ...
      <div style={{ position: 'absolute', inset: 0, opacity: 0.03, backgroundImage: 'repeating-linear-gradient(45deg, rgba(251,146,60,0.3) 0px, transparent 2px, transparent 20px, rgba(251,146,60,0.3) 22px)', backgroundSize: '30px 30px' }} />
      <div style={{ position: 'absolute', top: 0, left: '50%', width: '900px', height: '900px', background: 'radial-gradient(circle, rgba(251,146,60,0.04) 0%, transparent 65%)', transform: 'translate(-50%, -40%)' }} />
      ...
      <div className="container" style={{ position: 'relative', zIndex: 2, maxWidth: '1100px' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: '4rem' }}>
          <motion.div {...fadeIn}>
            <div style={{ display: 'inline-flex', alignItems: 'center', padding: '6px 14px', background: 'rgba(251,146,60,0.1)', border: '1px solid rgba(251,146,60,0.25)', borderRadius: '20px', fontSize: '0.75rem', color: '#fb923c', fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: '1.5rem' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '1.5rem', marginBottom: '3rem' }}>
          {[
            { icon: <Fingerprint size={22} />, title: 'Vehicle Custody Chain', desc: 'Every handoff — intake to lot, lot to lane, lane to transport — is recorded as a signed, timestamped transition. The full chain of custody is provable and immutable.', color: '#fb923c' },
            { icon: <FileCheck size={22} />, title: 'Condition Certificates', desc: 'OBD-II scan results from Lume-Auto are sealed on-ledger at the moment of diagnosis. The same data that generated the report is cryptographically bound to it — forever.', color: '#fb923c' },
            { icon: <ShieldCheck size={22} />, title: 'Arbitration Defense', desc: 'When a dispute arises, the ledger provides deterministic replay. Same inputs, same organism, same result — backed by a cryptographic proof that the data was never altered.', color: '#fb923c' },
          ].map((item, i) => (
            <motion.div key={i} {...fadeIn} transition={{ delay: i * 0.1 }}
              style={{ padding: '2rem', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(251,146,60,0.12)', borderRadius: '16px', transition: 'border-color 0.3s' }}
              onMouseEnter={e => (e.currentTarget as HTMLElement).style.borderColor = 'rgba(251,146,60,0.4)'}
              onMouseLeave={e => (e.currentTarget as HTMLElement).style.borderColor = 'rgba(251,146,60,0.12)'}>
              <div style={{ color: '#fb923c', marginBottom: '1rem' }}>{item.icon}</div>
        ...
        <motion.div {...fadeIn} transition={{ delay: 0.3 }} style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '2rem', marginBottom: '3rem' }}>
          <div className="panel" style={{ padding: '2.5rem', borderColor: 'rgba(251,146,60,0.15)', background: 'linear-gradient(180deg, rgba(251,146,60,0.03) 0%, transparent 100%)' }}>
            <h3 style={{ fontSize: '1.3rem', marginBottom: '1rem', color: '#fb923c', display: 'flex', alignItems: 'center', gap: '10px' }}>
            ...
            <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: '10px' }}>
              {[
                ...
                <li key={i} style={{ display: 'flex', alignItems: 'center', gap: '10px', fontSize: '0.9rem', color: 'var(--text-main)' }}>
                  <div style={{ width: 6, height: 6, borderRadius: '50%', background: '#fb923c', flexShrink: 0 }} /> {item}
                </li>
              ))}
            </ul>
          </div>
        ...
        {/* Bottom CTA */}
        <motion.div {...fadeIn} transition={{ delay: 0.4 }} className="panel" style={{ padding: '3rem', textAlign: 'center', borderColor: 'rgba(251,146,60,0.25)', background: 'linear-gradient(180deg, rgba(251,146,60,0.04) 0%, rgba(251,146,60,0.01) 100%)', boxShadow: '0 10px 40px rgba(251,146,60,0.04)' }}>
          <Server size={28} style={{ color: '#fb923c', marginBottom: '1rem' }} />
          <h3 style={{ fontSize: '1.5rem', marginBottom: '1rem', color: '#fb923c' }}>Your Data. Your Network. Your Ledger.</h3>
          <p className="text-muted" style={{ maxWidth: '750px', margin: '0 auto', fontSize: '1.05rem', lineHeight: 1.7 }}>
