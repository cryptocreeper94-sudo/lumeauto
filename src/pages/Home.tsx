          ...
          {[
            ...
            <motion.div key={i} initial={{ opacity: 0, y: 15 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="panel flex flex-col gap-3">
              <div className="flex items-center gap-3">
                <div style={{ color: 'var(--accent-cyan)' }}>{org.icon}</div>
                <h3 style={{ fontSize: '1.1rem' }}>{org.name}</h3>
              </div>
              <p className="text-muted" style={{ fontSize: '0.9rem' }}>{org.desc}</p>
              <span style={{ fontSize: '0.75rem', fontWeight: 600, color: org.statusColor, textTransform: 'uppercase', letterSpacing: '0.05em' }}>● {org.status}</span>
            </motion.div>
          ))}
        </div>
      </section>
