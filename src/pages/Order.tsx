...
const FEATURES = [
  { icon: <Activity size={20} />, title: '42-Signal Governance Engine', desc: 'Reads 42 OBD-II signals at 100ms intervals. RPM, fuel flow, combustion timing, air-fuel ratio â€” everything a $200 scanner shows and more.', color: 'var(--accent-cyan)' },
  ...
  { icon: <Shield size={20} />, title: 'Predictive Maintenance', desc: 'Detects component degradation 1â€“3% MPG before it triggers a check engine light. Fix $30 parts before they become $500 repairs.', color: '#38bdf8' },
...
const FAQS = [
  ...
  { q: 'How much will I actually save?', a: 'At $3.50/gallon and 15,000 miles/year, most drivers save $180â€“$320 annually. The kit pays for itself in under 3 weeks.' },
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(340px, 1fr))', gap: '3rem', alignItems: 'center' }}>
            ...
            <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }}>
              <div style={{ display: 'inline-block', padding: '6px 16px', background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.3)', borderRadius: '20px', fontSize: '0.75rem', color: '#ef4444', fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: '1.5rem' }}>
                âœ¦ {SAVINGS_PCT}% Off Retail Value
              </div>
              ...
              <p className="text-muted" style={{ fontSize: '1.1rem', maxWidth: '520px', lineHeight: 1.7, marginBottom: '1.5rem' }}>
                Professional-grade OBD-II diagnostics + deterministic fuel governance. Everything a $200 scanner does â€” <strong style={{ color: 'var(--text-main)' }}>plus an AI-free engine that saves you $180â€“$320/year on gas.</strong>
              </p>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.3 }}>
              ...
              <p className="text-dim" style={{ fontSize: '0.7rem', textAlign: 'center', marginTop: '0.75rem', letterSpacing: '0.05em' }}>
                LUME-AUTO DIAGNOSTIC KIT Â· PROFESSIONAL-GRADE HARDWARE + SOFTWARE
              </p>
            </motion.div>
          </div>
        </div>
        ...
        <div className="container" style={{ maxWidth: '900px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }}>
            ...
            <div className="panel" style={{ padding: '2rem', borderColor: 'rgba(239,68,68,0.2)' }}>
              ...
              <div style={{ fontSize: '2.5rem', fontWeight: 800, fontFamily: 'var(--font-mono)', color: '#ef4444', marginBottom: '1rem' }}>$149â€“$299</div>
              {['Read diagnostic codes', 'Clear check engine light', 'Basic live data', 'âŒ No fuel coaching', 'âŒ No predictive maintenance', 'âŒ No driver scoring', 'âŒ No family dashboard', 'âŒ Static â€” no intelligence'].map((item, i) => (
                <div key={i} style={{ fontSize: '0.85rem', color: item.startsWith('âŒ') ? 'var(--text-dim)' : 'var(--text-muted)', padding: '4px 0' }}>{item}</div>
              ))}
            </div>
            ...
            <div className="panel" style={{ padding: '2rem', borderColor: 'rgba(16,185,129,0.3)', background: 'rgba(16,185,129,0.03)' }}>
              ...
              {['âœ… Read diagnostic codes', 'âœ… Clear check engine light', 'âœ… 42 live signals at 100ms', 'âœ… Passive fuel coaching', 'âœ… Predictive maintenance', 'âœ… Driver efficiency scoring', 'âœ… Family & fleet dashboard', 'âœ… deterministic engine intelligence'].map((item, i) => (
      ...
      <section style={{ padding: '5rem 0', background: 'rgba(255,255,255,0.01)', borderBottom: '1px solid var(--border-light)' }}>
        <div className="container">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1.5rem' }}>
            {[
              { icon: <Package size={28} />, title: 'WiFi OBD-II Adapter', desc: 'Professional-grade hardware. Plugs into any vehicle sold after 1996. WiFi connectivity â€” works with iPhone and Android.', color: 'var(--accent-cyan)' },
      ...
      <section style={{ padding: '5rem 0', background: 'rgba(255,255,255,0.01)', borderBottom: '1px solid var(--border-light)' }}>
        <div className="container" style={{ maxWidth: '700px' }}>
          ...
          <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }}
            ...
            <div className="text-muted" style={{ fontSize: '0.95rem' }}>Kit costs ${KIT_PRICE} Â· <strong style={{ color: 'var(--text-main)' }}>Pays for itself in {Math.ceil(KIT_PRICE / (328/365))} days</strong></div>
          </motion.div>
        </div>
      </section>
        ...
        <div className="container">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            ...
            <button onClick={handleCheckout} disabled={loading} className="btn-primary" style={{
              ...
              {loading ? 'Redirecting...' : <><ShoppingCart size={20} /> Order Now â€” ${KIT_PRICE}</>}
            </button>
